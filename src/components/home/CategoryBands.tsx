...
export const CategoryBands: React.FC = () => {
  ...
  return (
    <section className="bg-[#090808] text-white py-12 md:py-16 border-b border-[#302F2E]">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-12">
        <div className={`grid gap-8 items-center ${gridColsClass}`}>
          {visibleCategories.map((category) => {
            ...
            const alt = category.altText || `${category.name} category collection`

            return (
              <Link
                key={category.id}
                to={destUrl}
                className="group flex items-center space-x-4 sm:space-x-5 p-2 transition-transform duration-200 hover:-translate-y-1"
              >
                {/* Thumbnail image with subtle zoom */}
                <div className="w-20 h-24 sm:w-24 sm:h-28 flex-shrink-0 overflow-hidden bg-[#302F2E] border border-[#BEBDBB]/20 shadow-sm">
                  <picture>
                ...
                <div className="flex-1 space-y-1 text-left">
                  <h3 className="font-display text-lg sm:text-xl font-bold tracking-wider uppercase text-white group-hover:text-[#BEBDBB] transition-colors">
                    {category.name}
                  </h3>
                  ...
                  <div className="pt-2 flex items-center gap-1.5 text-[11px] font-bold tracking-widest text-white uppercase group-hover:underline underline-offset-4">
                    <span>{ctaText}</span>
